...
import { MIN_AGE, MAX_AGE, HttpCode } from '../../../lib/constants';
...
const querySchema = Joi.object({
    ...
    sortByDecs: Joi.string().valid('name', 'age', 'email').optional(),
    filter: Joi.string()
        // eslint-disable-next-line prefer-regex-literals
        .pattern(new RegExp('(name|age|email)\\|?(name|age|email)+'))
        .optional()
});
...
export const validateUpdate = async (req, res, next) => {
    try {
        ...
        return res.status(HttpCode.NOT_FOUND).json({ message: err.message })
    }
    next()
};
...
export const validateId = async (req, res, next) => {
    if (!Types.ObjectId.isValid(req.params.id)) {
        return res.status(HttpCode.NOT_FOUND).json({ message: 'Invalid ObjectId' })
    }
    next()
};
